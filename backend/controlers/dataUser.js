...
async function dataUserDenegado(dni){
    
    try {
        const{rows}=await pool.query(`SELECT * FROM vista_asociado_denegado WHERE dni_persona = $1`, [dni]);
        ...
        const userDataArray = rows.map(row => ({
            ...
            respuesta_solicitud: row.respuesta_solicitud,
            fecha_aprobacion:row.fecha_aprobacion,
            detalle_solicitud:row.detalle_solicitud
        }));
        return userDataArray;
...
module.exports = { dataUserAprobado,dataUserDenegado };
